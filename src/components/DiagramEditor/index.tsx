...
import { DiagramComponent,  SnapSettingsModel,  OverviewComponent,  GridlinesModel,  Inject,  ConnectorModel,  NodeModel,  DiagramTools,  UndoRedo,  DataBinding,  DiagramContextMenu,  Keys,  KeyModifiers,  CommandManagerModel,  UserHandleModel,  UserHandleEventsArgs,  SelectorConstraints,  Snapping,  DiagramConstraints,  DiagramModel,  Connector,  ComplexHierarchicalTree,  LayoutModel } from '@syncfusion/ej2-react-diagrams';
...
import { getConnectorCornerRadius,  getConnectorType,  getFirstSelectedNode,  getGridColor,  getGridType,  getNodeConfig,  getPortOffset,  getPortSide,  getSnapConstraints,  initializeNodeDimensions,  isConnectorBetweenAgentAndTool,  isNodeOutOfViewport,  isStickyNote,  prepareUserHandlePortData,  updateNodeConstraints,  shouldShowUserHandleForPort,  isAgentBottomToToolConnector,  computeConnectorLength,  adjustUserHandlesForConnectorLength,  buildNodeHtml,  attachNodeTemplateEvents } from '../../utilities';
...
import { initializeStickyNote, handleStickyNoteEditMode } from '../../utilities/stickyNoteUtils';
...
function buildUserHandles(): UserHandleModel[] {
  return [
    {
...
function updateNodeTemplates(node: NodeModel, diagramRef: React.RefObject<DiagramComponent | null>) {
...
function updateNodeSelection(nodeIds: string[] | null) {
...
function updateResizeHandleVisibility(nodeIds: string[], diagramRef: React.RefObject<DiagramComponent | null>) {
  const diagram = diagramRef?.current;
  if (!diagram) return;
